...
注释：因为packet的data是IPv4头，这里没有MAC，所以这里填写虚假的目的MAC，但也可发送出去。
*/
{
...
void InitIpv4HeaderForTcp(PIPV4_HEADER in_ipv4, OUT PRAW_TCP buffer)
/*
...
*/
{
    ...
    BYTE version = in_ipv4->Version;
    ASSERT(IPV4_VERSION == version);
...
#pragma warning( pop )
    ...
    out_ipv4->Protocol = IPPROTO_TCP;
    out_ipv4->SourceAddress.S_un.S_addr = in_ipv4->DestinationAddress.S_un.S_addr;
    out_ipv4->DestinationAddress.S_un.S_addr = in_ipv4->SourceAddress.S_un.S_addr;
    out_ipv4->HeaderChecksum = checksum((unsigned short *)out_ipv4, sizeof(IPV4_HEADER));
}


void InitTcp4Header(PIPV4_HEADER ipv4, OUT PRAW_TCP buffer)
{
...
void PacketizeAck4(IN PIPV4_HEADER in_ipv4, IN PBYTE SrcMac, OUT PRAW_TCP buffer)
{
    ...
    InitIpv4HeaderForTcp(in_ipv4, buffer);
    InitTcp4Header(in_ipv4, buffer);
...
注释：因为packet的data是IPv6头，这里没有MAC，所以这里填写虚假的目的MAC，但也可发送出去。
*/
{
    buffer->eth_hdr.Destination.Byte[0] = 0x99;
    buffer->eth_hdr.Destination.Byte[1] = 0x99;
...
void InitIpv6HeaderForTcp(PIPV6_HEADER in_ipv6, OUT PRAW6_TCP buffer)
/*
...
#pragma warning(suppress:4366) //一元“&”运算符的结果可能是未对齐的
    ...
    ipv6_hdr->NextHeader = IPPROTO_TCP;
    ipv6_hdr->HopLimit = 128;

    RtlCopyMemory(&ipv6_hdr->SourceAddress, &in_ipv6->DestinationAddress, sizeof(IN6_ADDR));
    RtlCopyMemory(&ipv6_hdr->DestinationAddress, &in_ipv6->SourceAddress, sizeof(IN6_ADDR));
}


void InitTcp6Header(PIPV6_HEADER ipv6, OUT PRAW6_TCP buffer)
//和ipv4差不多。
{
    ASSERT(buffer);
...
void PacketizeAck6(IN PIPV6_HEADER ipv6, IN PBYTE SrcMac, OUT PRAW6_TCP buffer)
{
    ...
    InitIpv6HeaderForTcp(ipv6, buffer);
    InitTcp6Header(ipv6, buffer);
