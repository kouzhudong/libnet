...
*/
{
    ...
    //获取大小。
    bool b = InternetGetCookie(szURL, nullptr, lpszData, &dwSize);
    if (!dwSize && b == false) {
    ...
    b = InternetGetCookie(szURL, nullptr, lpszData, &dwSize);
    ...
    b = InternetSetCookie(szURL, nullptr, TEXT("TestData = Test")); //这个方式，最后一个参数里面必须有等号。
    b = InternetSetCookie(
        szURL, nullptr, TEXT("TestData = Test; expires = Sat,01-Jan-2015 00:00:00 GMT")); //显示用的。

    b = InternetSetCookie(szURL, L"QQ", TEXT("112426112")); //这个会自动加等号。
    // b = InternetSetCookie(szURL, nullptr, TEXT("QQ = 112426112"));
    b = InternetSetCookie(szURL, nullptr, TEXT("QQ = 112426112; expires = Sat,01-Jan-2015 00:00:00 GMT"));
    ...
    */
    ...
    if (b) {
        MessageBox(0, sz_cookie, L"CSIDL_COOKIES:", 0); //或者直接打开目录。cookie的目录是
    }
