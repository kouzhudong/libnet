...
icmp校验和计算心得：
1.必须的。
  《TCPIP详解卷一》如是说：ICMP的检验和是必需的
2.和IP校验和类似。
  注意不是一样：《TCPIP详解卷一》说相同，其实不然，不信你编写算法和wireshark的比较下。
  IP的校验和只计算头，没有后面的内容。
  ICMP的不止有ICMP的头，还有后面的内容。
3.根据第二条得出：
  修改IP头，不影响ICMP的校验和。
  只有修改ICMP的头及后面的内容才会且有必要重新计算校验和。
4.所以ICMP的计算很简单。
  首先置PICMP_HEADER的Checksum为0.
  第二步直接调用checksum(PICMP_HEADER, sizeof(ICMP_HEADER) + 后面的数据的大小);
  没有第三步。
*/
